...
def test_booking_conflicts():
    ...
    try:
        ...
        start_time1 = datetime.combine(
            tomorrow, datetime.strptime("09:00", "%H:%M").time()
        )
        end_time1 = datetime.combine(
            tomorrow, datetime.strptime("12:00", "%H:%M").time()
        )
        ...
        print(
            f"✓ Test 1 PASSED: Created first booking {start_time1.strftime('%H:%M')} - {end_time1.strftime('%H:%M')}"
        )
        ...
        end_time2 = datetime.combine(
            tomorrow, datetime.strptime("13:00", "%H:%M").time()
        )
        ...
        end_time3 = datetime.combine(
            tomorrow, datetime.strptime("17:00", "%H:%M").time()
        )
        ...
        try:
            ...
            print(
                f"✓ Test 3 PASSED: Non-overlapping booking created {start_time3.strftime('%H:%M')} - {end_time3.strftime('%H:%M')}"
            )
        except Exception as e:
        ...
        start_time4 = datetime.combine(
            tomorrow, datetime.strptime("12:00", "%H:%M").time()
        )
        end_time4 = datetime.combine(
            tomorrow, datetime.strptime("14:00", "%H:%M").time()
        )
        ...
        try:
            ...
            print(
                f"✓ Test 4 PASSED: Adjacent booking created {start_time4.strftime('%H:%M')} - {end_time4.strftime('%H:%M')}"
            )
        except Exception as e:
        ...
        for booking in final_bookings:
            print(
                f"  {booking.start_date.strftime('%H:%M')} - {booking.end_date.strftime('%H:%M')} ({booking.status})"
            )
